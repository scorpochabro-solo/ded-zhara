...
| `YANDEX_CALDAV_USER` / `YANDEX_CALDAV_PASSWORD` | Логин и «пароль приложения» Яндекса — записи падают в Яндекс Календарь (CalDAV). Пусто = выключено. | нет |
| `YANDEX_CALDAV_CALENDAR` | Имя календаря (пусто — первый/основной). | нет |
...
> **Яндекс Календарь.** При заданных `YANDEX_CALDAV_USER` / `YANDEX_CALDAV_PASSWORD` каждая заявка создаёт событие в Яндекс Календаре (имя, услуга, телефон, желаемые дата и время). Пароль — это **«пароль приложения»** из Яндекс ID → Безопасность → Пароли приложений → «Календарь CalDAV» (не основной пароль аккаунта). Работает только на боевом сервере с бэкендом (Vercel/VPS), не на статичном демо GitHub Pages. Если календарь недоступен — заявка всё равно сохранится в БД.
